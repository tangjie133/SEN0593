...
{
  if(__HAL_LVD_GET_FLAG(hlvd) != RESET)	// 发生LVD中断 
  {
    if(__HAL_LVD_GET_IT_SOURCE(hlvd) !=RESET)		// LVD中断使能 
    {
...
{	
	...
	if(hlvd->Init.FltClkSel != LVD_FLTCLK_DISABLE)
	{
		...
		__HAL_LVD_ENABLE_DIGFLT(hlvd);	//使能数字滤波  LVD_CR_FLTEN
	}
}
