...
HAL_StatusTypeDef HAL_OWIRE_Init(OWIRE_HandleTypeDef *howire)
{
  ...
  uint32_t cnt = HAL_RCC_GetPCLKFreq()/1000000;
	...
	howire->Instance->RSTCNT = howire->Init.ResetCounter*cnt; // 主发送复位时间设定计数值
	howire->Instance->PRESCNT = howire->Init.PresencePulseCounter*cnt; // 从应答时间设定计数值 
	howire->Instance->BITRATECNT = howire->Init.BitRateCounter*cnt; // Bit Rate时间设定计数值 
	howire->Instance->DRVCNT = howire->Init.DriveCounter*cnt;  // 主器件读/写PULL0驱动时间设定计数值 
	howire->Instance->RDSMPCNT = howire->Init.ReadSamplingCounter*cnt; // 主器件读采样时间设定计数值 
	howire->Instance->RECCNT = howire->Init.RecoverCounter*cnt; // Recover Time计数区间值 

	/* Enable the OWIRE peripheral */
	howire->Instance->CR |= OWIRE_CR_EN_Msk;	// 1-wire模块使能
...
//			howire->ErrorCode = HAL_OWIRE_ERROR_NOACK; // 如需要可打开
//			return HAL_ERROR;
		}
